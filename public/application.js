...
/***/ (function(module, exports, __webpack_require__) {
	...
	var onSignUp = function onSignUp(event) {
	  ...
	  api.signUp(data).done(function (response) {
	    var sdata = {
	      credentials: {
	    ...
	    api.signIn(sdata).done(ui.signInSuccess).catch(ui.signInError);
	...
	var onSignIn = function onSignIn(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.signIn(data).done(ui.signInSuccess).catch(ui.signInError);
	};

	var onSignOut = function onSignOut(event) {
	  event.preventDefault();
	  if (store.user) {
	    api.signOut().done(ui.signOutSuccess).catch(ui.signOutError);
	  } else {}
	};
	...
	var onNewGame = function onNewGame(event) {
	  event.preventDefault();
	  if (store.user) {
	    api.createGame().done(game.openGame).catch(function () {});
	  } else {}
	};
	...
	var onGetAllGames = function onGetAllGames(event) {
	  event.preventDefault();
	  if (store.user) {
	    api.getGames().done(game.retrieveGames).catch(function () {});
	  } else {}
	};
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var getGameID = function getGameID(id) {
	  return $.ajax({
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var forceNewGame = function forceNewGame() {
	  if (!store.currentGame) {
	    $('#-games-modal').modal('show');
	  }
	};

	var gameWon = function gameWon(playerTwo) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	// Player colors
	var playerOneColor = '#ff8800';
	var playerTwoColor = '#0044ff';
	var emptyColor = '#e5e5e5';

	// Status display
	var gameStatusElement = $('.game-status');
	var gameStatusHeader = gameStatusElement.children('h1');

	// API success handlers
	var retrieveGames = function retrieveGames(response) {
	  var games = response.games;
	  var stats = evaluateGames(response);
	...
	var openGame = function openGame(response) {
	  if (!response.game || response.game.over) {
	    return false;
	  }
	...
	var evaluateGame = function evaluateGame(gameData, playerTwo) {
	  var cells = gameData.cells;
	  if (checkHorizontal(cells, 0, playerTwo) || checkVertical(cells, 0, playerTwo) || checkHorizontal(cells, 1, playerTwo) || checkVertical(cells, 1, playerTwo) || checkHorizontal(cells, 2, playerTwo) || checkVertical(cells, 2, playerTwo) || checkDiagonalTopRight(cells, playerTwo) || checkDiagonalTopLeft(cells, playerTwo)) {
	    return true;
	  } else {
	    return false;
	  }
	...
	var evaluateGames = function evaluateGames(response) {
	  var record = {
	    win: 0,
	    loss: 0,
	    draw: 0,
	...
	var mapGame = function mapGame(gameData) {
	  if (gameData.over) {
	    return false;
	  }
	  var board = newGame();
	  for (var i = 0; i < gameData.cells.length; i++) {
	    var token = gameData.cells[i];
	    if (token) {
	...
	var sendMove = function sendMove(board, index, playerTwo) {
	  var over = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
	  ...
	  if (store.currentGame) {
	    ...
	    api.updateGame(store.currentGame.id, data).done(function (response) {});
	  } else {}
	};
	// HELPERS
	// Helper for getting token
	var getToken = function getToken(playerTwo) {
	  return playerTwo ? 'o' : 'x';
	};

	// Helper to change game status display
	var displayTurn = function displayTurn(playerTwo) {
	...
	var changeCell = function changeCell(board, index, playerTwo) {
	  board[index] = getToken(playerTwo);
	  var cell = $('.game-cell[data-id="' + index + '"]');
	...
	var getBoardSize = function getBoardSize(board) {
	  var dim = -1;
	...
	var maxTurns = gameBoard.length;

	var checkHorizontal = function checkHorizontal(board, row, playerTwo) {
	  var size = getBoardSize(board);
	  var y = row * size;
	  var token = getToken(playerTwo);
	  if (!token) {
	    return false;
	  }
	...
	var checkVertical = function checkVertical(board, col, playerTwo) {
	  var size = getBoardSize(board);
	  var token = getToken(playerTwo);
	  if (!token) {
	    return false;
	...
	var checkDiagonalTopLeft = function checkDiagonalTopLeft(board, playerTwo) {
	  var size = getBoardSize(board);
	  var token = getToken(playerTwo);
	  if (!token) {
	    return false;
	...
	var checkDiagonalTopRight = function checkDiagonalTopRight(board, playerTwo) {
	  var size = getBoardSize(board);
	  var sizem = size - 1;
	  var token = getToken(playerTwo);
	  if (!token) {
	...
	var attemptMove = function attemptMove(board, index, playerTwo) {
	  if (board[index]) {
	    return false;
	  }
	...
	var gameWon = function gameWon(playerTwo) {
	  ...
	  api.getGames().done(retrieveGames).catch(function () {});
	  newGame();
	};
	...
	var makeMove = function makeMove(index) {
	  if (attemptMove(gameBoard, index, playerTwoTurn)) {
	    ...
	    var col = index - row * size;
	    turnCount++;
	    if (checkHorizontal(gameBoard, row, playerTwoTurn) || checkVertical(gameBoard, col, playerTwoTurn) || checkDiagonalTopRight(gameBoard, playerTwoTurn) || checkDiagonalTopLeft(gameBoard, playerTwoTurn)) {
	      // Player won
